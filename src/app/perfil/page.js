// RUTA: src/app/perfil/page.js
"use client";

import { useEffect, useState, useMemo } from 'react';
import { useRouter } from 'next/navigation';
import { signOut } from "firebase/auth";
import { doc, setDoc, onSnapshot, collection, addDoc, query, orderBy, where, Timestamp } from "firebase/firestore";
...
// Cada uno con su lógica y diseño restaurados.

const MiCuenta = ({ user }) => (
    <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
        <h2 className="text-2xl mb-4 font-bold text-white">Tus Datos</h2>
        <div className="space-y-3">
            <p className="text-gray-400">Email: <span className="font-medium text-white">{user.email}</span></p>
            {user.rol && <p className="text-gray-400 mt-2">Rol: <strong className={`font-medium capitalize px-2 py-1 text-sm rounded-full ${user.rol === 'admin' ? 'bg-indigo-500' : 'bg-gray-600'}`}>{user.rol}</strong></p>}
            {user.plan && <p className="text-gray-400 mt-2">Plan: <strong className="font-medium capitalize text-white bg-green-600 px-2 py-1 rounded-full text-sm">{user.plan}</strong></p>}
        </div>
    </div>
);
