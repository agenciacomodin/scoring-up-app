        ...
        <div className="bg-gray-800 p-6 rounded-lg">
            ...
            <form onSubmit={handleS}>
                <div>
                    ...
                    <input type="text" value={nombre} onChange={(e)=>setNombre(e.target.value)} required className="w-full mt-1 p-2 rounded-lg bg-gray-700"/>
                </div>
                <div className="mt-4">
                    <label>DNI</label>
                    <input type="number" value={dni} onChange={(e)=>setDni(e.target.value)} required className="w-full mt-1 p-2 rounded-lg bg-gray-700"/>
                </div>
...
const MisInformes = ({ user, solicitud, loadingSolicitud }) => {
    const handleDescargar = async () => { 
        try { 
            ...
            if(error.code === 'storage/object-not-found') {
                ...
                alert("No se pudo descargar el informe en este momento.");
            }
        }
    };
    if (loadingSolicitud) return <div className="bg-gray-800 p-6 rounded-lg text-center"><p>Cargando información de informes...</p></div>;
    return (
        <div className="space-y-6">
            ...
            <div className="bg-gray-800 p-6 rounded-lg">
                ...
                {!solicitud ? (
                    <p className="text-gray-400">No has realizado ninguna solicitud.</p>
                ) : (
                    <div className="space-y-4">
                        ...
                        {(solicitud.estado === 'Completado' || solicitud.estado === 'informe_enviado') && (
                            <button onClick={handleDescargar} className="w-full mt-2 bg-purple-600 p-3 rounded-lg font-bold">Descargar Informe (PDF)</button>
                        )}
                    </div>
                )}
            </div>
...
const Educacion = () => {
    const cats = [{n:'Deudas', l:[{t:'Leer Veraz'}]},{n:'Ahorro',l:[{t:'Método 50/30/20'}]}]; 
    return(<div className="bg-gray-800 p-6 rounded-lg"><h2 className="text-2xl mb-4">Educación</h2>{cats.map(c=><div key={c.n}><h3 className="text-lg mt-4 font-bold text-green-400">{c.n}</h3><ul>{c.l.map(l=><li key={l.t} className="p-2 bg-gray-700 mt-2 rounded-lg hover:bg-gray-600" onClick={()=>alert("Proximamente...")}>{l.t}</li>)}</ul></div>)}</div>)
};
...
const Billetera = ({ user, solicitud }) => {
    ...
    const agregarMovimiento = async (e) => { 
        e.preventDefault(); 
        await addDoc(collection(db,'users',user.uid,'movimientos'),{descripcion,monto:tipo==='ingreso'?Number(monto):-Number(monto),tipo,categoria:tipo==='ingreso'?'ingresos':categoria,fecha:new Date()}); 
        setDescripcion(''); setMonto('');
    };

    const { balance, totalIngresos, totalEgresos } = useMemo(() => {
        ...
        return { balance: ingresos + egresos, totalIngresos: ingresos, totalEgresos: -egresos };
    }, [movimientos]);
        ...
        <div className="space-y-8">
            ...
            <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
                ...
                <div className="flex justify-between mt-4 text-sm">
                    <div className="text-green-400"><p>Ingresos</p><p>${totalIngresos.toLocaleString('es-AR')}</p></div>
                    <div className="text-red-400"><p>Egresos</p><p>${totalEgresos.toLocaleString('es-AR')}</p></div>
                </div>
            </div>
            <div className="bg-gray-800 p-6 rounded-lg">
...
// --- COMPONENTE PRINCIPAL (Reestructurado para usar el AuthContext) ---
export default function PerfilPage() {
    ...
    // El menú ahora se define aquí para tener acceso a todo
    const MenuNav = () => (
        <nav className="flex flex-col space-y-2 p-4">
            {currentUser.rol === 'admin' && (
                <a href="/admin" className="w-full text-left p-3 rounded-lg bg-indigo-600 hover:bg-indigo-700 font-bold text-center mb-2">
                    ★ IR A PANEL ADMIN ★
                </a>
            )}
    ...
    if (authLoading || !currentUser) { 
        return <div className="bg-gray-900 text-white min-h-screen flex items-center justify-center"><p>Cargando sesión...</p></div>; 
    }
        ...
        <div className="min-h-screen bg-gray-900 text-white"> 
            ...
            <div className="flex"> 
                ...
                <main className="flex-1 p-4 md:p-8 overflow-y-auto"> 
                    ...
                    {activeTab === 'informes' && <MisInformes user={currentUser} solicitud={solicitud} loadingSolicitud={solicitudLoading} />}
