// CÓDIGO COMPLETO PARA src/app/perfil/page.js
...
const MiCuenta = ({ user }) => ( <div className="bg-gray-800 p-6 rounded-lg"><h2 className="text-xl mb-4 font-bold">Tus Datos</h2><p className="text-gray-400">Email: <span className="text-white">{user.email}</span></p>{user.rol && <p className="text-gray-400">Rol: <strong className="text-white">{user.rol}</strong></p>}</div>);
const SolicitarInforme = ({ user }) => { const [nombre, setNombre] = useState(""); const [dni, setDni] = useState(""); const handleS = async (e) => { e.preventDefault(); try { await setDoc(doc(db, "solicitudes", user.uid),{nombre,dni,email:user.email,estado:"pendiente_pago",fecha:new Date()}); window.location.href="https://mpago.la/2ADsZ18";} catch(e){console.error(e)} }; return ( <div className="bg-gray-800 p-6 rounded-lg"><h2 className="text-2xl font-bold mb-4">Solicitar Informe</h2><form onSubmit={handleS}><div><label>Nombre</label><input type="text" value={nombre} onChange={(e)=>setNombre(e.target.value)} required className="w-full mt-1 p-2 rounded-lg bg-gray-700"/></div><div className="mt-4"><label>DNI</label><input type="number" value={dni} onChange={(e)=>setDni(e.target.value)} required className="w-full mt-1 p-2 rounded-lg bg-gray-700"/></div><button type="submit" className="w-full mt-6 bg-green-600 p-3 rounded-lg font-bold">Pagar</button></form></div> )};
const MisInformes = ({ user, solicitud, loadingSolicitud }) => { const handleDescargar = async () => { try { const {getStorage,ref,getDownloadURL}=await import("firebase/storage");const storage=getStorage(); const filePath=`informes-pendientes/${user.uid}.pdf`;const fileRef=ref(storage,filePath); const url=await getDownloadURL(fileRef);window.open(url,'_blank')}catch(error){ if(error.code === 'storage/object-not-found'){ alert("Tu informe aún no ha sido procesado.")}else{alert("No se pudo descargar.")}}}; if (loadingSolicitud) return <div className="bg-gray-800 p-6 rounded-lg text-center"><p>Cargando información de informes...</p></div>; return ( <div className="space-y-6"><ResumenCrediticio solicitud={solicitud} /> <div className="bg-gray-800 p-6 rounded-lg"><h2 className="text-2xl mb-4 font-bold">Detalle y Acciones</h2>{!solicitud ? ( <p className="text-gray-400">No has realizado ninguna solicitud.</p>) : ( <div className="space-y-4"><div className="bg-gray-700 p-4 rounded-lg"><p className="text-gray-400 text-sm">Fecha</p><p>{solicitud.fecha?.toDate().toLocaleDateString('es-AR')}</p><p className="text-gray-400 text-sm mt-2">Estado</p><p className="font-bold text-green-400 text-lg">{solicitud.estado}</p></div> {solicitud.resumenIA && (<div className="bg-gray-700 p-4 rounded-lg"><h3 className="text-lg font-semibold mb-2 text-green-300">Análisis IA</h3><p className="text-gray-300 whitespace-pre-wrap">{solicitud.resumenIA}</p></div>)} {(solicitud.estado === 'Completado' || solicitud.estado === 'informe_enviado') && (<button onClick={handleDescargar} className="w-full mt-2 bg-purple-600 p-3 rounded-lg font-bold">Descargar PDF</button>)}</div>)}</div></div>);};
...
const Billetera = ({ user, solicitud }) => {
    ...
    const { balance, dataGrafico, totalIngresos, totalEgresos } = useMemo(() => {
        ...
        const dataParaGrafico = Object.entries(movimientos
            .filter(m => m.tipo === 'egreso')
            .reduce((a,c) => { 
                const cat = c.categoria || 'otros';
                a[cat] = (a[cat] || 0) + -c.monto; return a; 
            }, {}))
            .map(([name,value]) => ({ name: name.charAt(0).toUpperCase() + name.slice(1), value }));
        
        return { balance: ingresos + egresos, dataGrafico: dataParaGrafico, totalIngresos: ingresos, totalEgresos: -egresos };
    }, [movimientos]);
